...
function renderMarkdown(markdown, moduleId) {
  ...
  while (index < lines.length) {
    ...
    if (line.startsWith("::: ")) {
      ...
      else output.push(`<aside class="${type === "visual" ? "visual-section" : "activity-card full-activity"}">${renderMarkdown(inner.join("\n"), moduleId)}</aside>`);
      continue;
    }
    if (/^!\[/.test(line)) {
