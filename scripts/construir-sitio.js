...
function shell({ title, description, body, pageClass = "", script = "", depth = 0 }) {
  const prefix = depth ? "../../" : "";
  return `<!doctype html><html lang="es"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><meta name="description" content="${esc(description)}"><title>${esc(title)} | Curso Esencias Naturales</title><link rel="stylesheet" href="${prefix}styles.css?v=v2-5">${script ? `<script src="${prefix}${script}?v=v2-5" defer></script>` : ""}</head><body class="${pageClass}"><a class="skip-link" href="#contenido">Saltar al contenido</a><header class="site-header"><a class="brand" href="${prefix}index.html"><span class="brand-mark" aria-hidden="true">E</span><span>Curso Esencias Naturales</span></a><span class="release-note">Partes 1 a 4 disponibles</span></header><main id="contenido">${body}</main><footer><span>Curso Esencias Naturales · De la planta a la esencia</span><a href="${prefix}index.html">Índice general</a></footer><dialog class="definition-dialog"><button type="button" aria-label="Cerrar">×</button><h2></h2><p></p></dialog></body></html>`;
}
...
console.log("Sitio V2 construido: índice, 6 partes, 19 módulos y 4 cierres integradores.");
