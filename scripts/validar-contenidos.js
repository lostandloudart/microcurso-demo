...
if(html.length!==17) errors.push(`Se esperaban 17 HTML y se encontraron ${html.length}.`);
...
for(const item of json.filter((i)=>/^p0[12]-/.test(String(i.id)))){for(const key of ["archivo_original","archivo_recortado","archivo_web"]){if(!fs.existsSync(path.join(root,"03_IMAGENES",item[key])))errors.push(`Falta imagen ${item[key]}`);}if(!item.texto_alternativo||!item.consigna_observacion)errors.push(`${item.id}: accesibilidad u observación incompleta.`);}
console.log(`Partes 1 y 2: ${moduleFiles.length} módulos; sitio: ${html.length} páginas; errores: ${errors.length}; advertencias: ${warnings.length}.`);
